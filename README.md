...
| [pkg-errors-wrap-nil-err](go/pkg-errors-wrap-nil-err.yaml) | [🛝🔗](https://semgrep.dev/playground/r/trailofbits.go.pkg-errors-wrap-nil-err.pkg-errors-wrap-nil-err) | 🟧 | 🌗 | `pkg/errors.Wrap`/`Wrapf`/`WithMessage`/`WithMessagef`/`WithStack` called where the error argument is provably nil — wraps to nil, swallowing the failure path |
